...
window.addEventListener("load", () => {
	...
	if (kategoriLista.length <= 4) {
		...
		for (let c = 0; c < 3; c++) {
			document
				...
				.appendChild(addMainMenuButton(kategoriLista[Object.keys(kategoriLista)[c]]));
		}
		document
			.querySelector("#topnav #top-nav-list")
	...
	if (window.location.href.includes("indexkat.html")) {
		...
		const produktAddToCartButton = document.querySelector("#produktAddToCartButton");
		const produktAddToCartValue = document.querySelector("#produktAddToCartValue");
